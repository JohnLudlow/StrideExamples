...
public class RaycastInteractionScript : AsyncScript
{
  ...
  public override async Task Execute()
  {
    ...
    while (Game.IsRunning)
    {
      ...
      if (Input.IsKeyDown(Stride.Input.Keys.LeftShift) && Input.IsMouseButtonDown(Stride.Input.MouseButton.Left))
      {
        ProcessMouseRaycast(cameraComponent);
      }
      else if (Input.IsMouseButtonPressed(Stride.Input.MouseButton.Left))
      {
        ProcessMouseRaycast(cameraComponent);
      }
  ...
  {
    var baseScore = numberOfCubes * Constants.BasePointsPerCube;

    var bonus = (numberOfCubes == 1 ? 0 : numberOfCubes) * numberOfCubes * 10;
